...
## 8/29/16: shift data one week ahead to account for incubation period and reporting delay
...
# shift = -7 (e.g., Christmas should have fallen on the week before)
interestWeeks_newDef <- ilic_df2 %>%
  distinct(week, year, month) %>%
  filter(month >= 11 | month <= 2) %>%
  mutate(actualXmas = ifelse(as.Date(paste0(year, "-12-25")) >= week & as.Date(paste0(year, "-12-25")) < week+7, week, NA)) %>%
  mutate(bdaIndicator = ifelse((week-7) %in% actualXmas, "d", NA)) %>%
  # mutate(bdaIndicator = ifelse(as.Date(paste0(year, "-12-25")) >= week-7 & as.Date(paste0(year-1, "-12-25")) < week+7-7, "d", NA)) %>%
  mutate(bdaIndicator = ifelse(as.Date(paste0(year, "-12-25")) >= week+21-7 & as.Date(paste0(year, "-12-25")) < week+21+7-7, "b", bdaIndicator)) %>%
  mutate(bdaIndicator = ifelse(as.Date(paste0(year-1, "-12-25")) >= week-21-7 & as.Date(paste0(year-1, "-12-25")) < week-21+7-7, "a", bdaIndicator)) %>%
  mutate(week2 = week+7) %>%
...
setwd(dirname(sys.frame(1)$ofile))
setwd("./import_data")
write_csv(exportDat, "ILIn_bdaIndicator_incidence_shifted.csv")
# exported 8/29/16
