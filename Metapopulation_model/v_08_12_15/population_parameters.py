...
###################################################
def calc_metro_age_pop (alpha):
#assign metro pop for child and adult based on alpha

    metropop = csv.reader(open(os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/metedges.txt'), 'r'),delimiter = '\t')
...
# calculate probability of child or adult traveling from metro i to metro j
    ...
    for (i, j) in edges:
        ...
        N_i = dict_metropop[i]
        N_j = dict_metropop[j]
        prob_child_travels_i_j = ((r / alpha) * (w_ij / N_i))
        prob_adult_travels_i_j = (((1 - r) / (1 - alpha)) * (w_ij / N_i))
        prob_child_travels_j_i = ((r / alpha) * (w_ij / N_j))
        prob_adult_travels_j_i = (((1 - r) / (1 - alpha)) * (w_ij / N_j))
...
#    alpha = 
#
#    d_metro_age_pop = calc_metro_age_pop(alpha)
