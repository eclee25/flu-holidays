...
metro_edges_travelers = os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/air_traffic_edgelist.txt')
...
popdata = csv.reader(open(os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/SDI_Data/totalpop_age.csv'), 'r'),delimiter = ',')

# import data into dicts
d_pop_for_yr_age, ages, years = func.import_popdata(popdata, 0, 1, 2)
# ECL 8/12: ages code commented out
...
d_metro_age_pop = func.calc_metro_age_pop(a)

metids = sorted(set([key[0] for key in d_metro_age_pop]))
pop_by_metid = [sum([d_metro_age_pop[key] for key in d_metro_age_pop if key[0]==id]) for id in metids]
