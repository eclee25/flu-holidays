...
# FALSE - because p includes across group but within children
...
# in order to calc avg p value for children and adults
    # from Germany data
    # at 5 to 10 year increment groups
...
if __name__ == "__main__":
    ...
    filename_germ_contact_data = os.path.abspath('../../../../Anne_Bansal_lab/Contact_Data/polymod_germany_contact_matrix_Mossong_2008.csv')
    ...
    print "proportion of US. pop that are C (alpha)", a
    ...
    print "all C contacts (q_c)", q_c
    print "all A contacts (q_a)", q_a
    print "fraction of C contacts w/ C (p_c)", p_c
    print "fraction of A contacts w/ A (p_a)", p_a
    print "# within-group contacts among C", C_cc
    print "# within-group contacts among A", C_aa

    C = calc_contact_matrix_pqa(p_c, p_a, q_c, q_a, a)

    print C

    # EL 10/17/15 Apollini2014 EQ2
    # leftside = rightside means that the contact matrix is symmetric (# contacts bw group 1 pop & group 2 pop is the same as the number of contacts between group 2 pop & group 1 pop)
    # i.e., Cij = Cji
    print 'leftside', (1-a)*(1-p_a)*(q_a/q_c)
    print 'rightside', a*(1-p_c)
