...
## Name: Elizabeth Lee
## Date: 10/5/16
## Function: national time series with partial school closure intervention (-10%, actual, +10%)
...
plotNatTS_contact <- function(dat, holidayTiming, exportPath){
  ...
  if (holidayTiming == 'actual'){
    holidayStart <- 90
    exportFilename <- paste0(exportPath, "/nationalContactrate_avg_actual_incidence.png")
  } 
...
setwd(dirname(sys.frame(1)$ofile)) # only works if you source the program
setwd("../R_export")
write_csv(fullDat2, "nationalContactrate_avg_allCombos_incidence.csv")
# 10/5/16