...
plotNatTS_RR <- function(dat, holidayTiming, age, exportPath){
  ...
  if (holidayTiming == 'actual'){
    holidayStart <- 90
    exportFilename <- paste0(exportPath, sprintf("/holidayNational%s_avg_actual.png", age))
  } else if (holidayTiming == 'plus three weeks'){
    holidayStart <- 111
    exportFilename <- paste0(exportPath, sprintf("/holidayNational%s_avg_late3wks.png", age))
  } else if (holidayTiming == 'plus six weeks'){
    holidayStart <- 132
    exportFilename <- paste0(exportPath, sprintf("/holidayNational%s_avg_late6wks.png", age))
  }
