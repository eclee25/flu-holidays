...
plotBxp_totBDA_contact <- function(dat, varname, varString, timingString, exportPath){
  ...
  exportFilename <- paste0(exportPath, sprintf("/bdaIncidence_contact_%s.png", timingString))
...
importDat <- read_csv("metroContactTot_avg_allCombos_incidence.csv") %>%
...
write_csv(exportDat, "metroContactTot_avg_allCombos_bda_IRR.csv")
# exported 10/5/16
