...
parseInfilename <- function(outputfilename){
  ...
  # intervention code positions (4 digits): contact (1 = red_C_all intervention, 2 = red_C_ageROnly intervention), travel (1 = swap_networks intervention), actual holiday timing (1 = actual, 0 = late holiday), 6 weeks forward (1 = 6 wks forward, 0 = 3 wks forward; NA if actual holiday timing = 1)
  
  contact <- ifelse(grepl("disease_red_C_all", outputfilename, fixed = TRUE), 1, ifelse(grepl("disease_red_C_ageROnly", outputfilename, fixed = TRUE), 2, 0))
...
parseOutfilename <- function(interventionCodes){
  ...
  contact <- ifelse(interventionCodes$contact, ifelse(interventionCodes$contact == 2, "disease_red_C_ageROnly", "disease_red_C_all"), "disease_none")
