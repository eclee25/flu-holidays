...
plotNatTS <- function(dat, holidayTiming, exportPath){
  ...
  if (holidayTiming == 'actual'){
    holidayStart <- 90
    exportFilename <- paste0(exportPath, "/national_avg_actual_incidence.png")
  } else if (holidayTiming == 'plus three weeks'){
    holidayStart <- 111
    exportFilename <- paste0(exportPath, "/national_avg_late3wks_incidence.png")
  } else if (holidayTiming == 'plus six weeks'){
    holidayStart <- 132
    exportFilename <- paste0(exportPath, "/national_avg_late6wks_incidence.png")
  }
  ...
  exportPlot <- ggplot(dat2, aes(x = time_step, y = infPer10K)) +
    geom_line(aes(colour = intervention), size = 1.3) + 
...
setwd(dirname(sys.frame(1)$ofile)) # only works if you source the program
setwd("../R_export")
write_csv(fullDat2, "national_avg_allCombos_incidence.csv")
write_csv(mergDat, "metroAge_avg_allCombos_incidence.csv")
# 8/26/16