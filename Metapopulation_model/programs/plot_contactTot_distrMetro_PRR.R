...
plotBxp_totBDA_contact <- function(dat, varname, varString, timingString, exportPath){
  ...
  exportFilename <- paste0(exportPath, sprintf("/bdaPrevalence_contact_%s.png", timingString))
...
importDat <- read_csv("metroContactTot_avg_allCombos_prevalence.csv") %>%
...
actualPlot <- plotBxp_totBDA_contact(actualBDA, "infPer10K", "flu prevalence per 10,000", "actual", path_export)
...
write_csv(exportDat, "metroContactTot_avg_allCombos_bda_PRR.csv")
# exported 8/12/16
