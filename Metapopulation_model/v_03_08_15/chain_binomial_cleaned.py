...
#  and return the total number of infected individuals (num_infected)
	...
	infc_id = [k[0] for k in d_metro_infected_child if d_metro_infected_child[k]>0]
	print '%s child pop' %(infc_id[0]), d_metro_age_pop[(infc_id[0], 'child')]
	print sorted([(k, v) for k, v in d_metro_infected_adult.items() if v > 0])
	print '%s adult pop' %(infc_id[0]), d_metro_age_pop[(infc_id[0], 'adult')]

	# # 8/13/15 ECL checks
	# print sorted([(key, d_metro_tot_infected_child[key]) for key in d_metro_tot_infected_child if key[1]<5])
	# print sorted([(key, d_tot_new_cases_child[key]) for key in d_tot_new_cases_child if key<5])
...
if __name__ == "__main__":
	...
	filename_metropop = '%sAnne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/metedges.txt' %(start_dir)
	d_metropop, metro_ids = pop_func.import_metropop(os.path.normpath
	(filename_metropop), 2, 3)
	filename_air_network = '%sAnne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/air_traffic_edgelist.txt' %(start_dir)
	air_network = read_edgelist_anne(os.path.normpath
	(filename_air_network))
	# READ US population data
	popfile = '%sAnne_Bansal_lab/SDI_Data/totalpop_age.csv' %(start_dir)
	us_popdata = csv.reader(open(os.path.normpath(popfile), 'r'), delimiter = ',')
	...
	filename_germ_contact_data = '%sAnne_Bansal_lab/Contact_Data/polymod_germany_contact_matrix_Mossong_2008.csv' %(start_dir)
	filename_germ_pop_data = '%sAnne_Bansal_lab/UNdata_Export_2008_Germany_Population.csv' %(start_dir)

		
	# DEFINE POPULATION PARAMETERS
	year = 2010
	alpha = pop_func.calc_alpha(year, dict_childpop, dict_adultpop)
	ch_travelers_r = 0.183 # fraction of children who travel
	
	# CONTACT MATRIX
	C = pop_func.calc_contact_matrix(os.path.normpath(filename_germ_contact_data), os.path.normpath(filename_germ_pop_data), alpha)
	
	# ECL 8/13/15: check when adult-child contact is 0
	# C = np.matrix([[C.item(0,0), 0], [0, C.item(1,1)]])
