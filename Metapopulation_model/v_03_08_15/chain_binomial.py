...
### functions ###
if os.uname()[:2] == ('Linux', 'bansallab'):
	start_dir = '/home/elee/Dropbox/'
elif os.uname()[:2] == ('Windows', 'garuda'):
	start_dir = 'C:/Users/Elizabeth/Dropbox (Bansal Lab)/'

# sys.path.append('%sElizabeth_Bansal_Lab/Anne/Metapopulation_model/v_03_08_15' %(start_dir))
# # Windows
# sys.path.append(os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Elizabeth_Bansal_Lab/Anne/Metapopulation_model/v_03_08_15'))
...
#  and return the total number of infected individuals (num_infected)
	...
	while num_infected > 0 and time_step < time_end:
		...
		for met_id in metro_ids:
			...
			# print 'tot I', time_step, met_id, d_metro_tot_infected_adult[(met_id, time_step)] + d_metro_tot_infected_child[(met_id, time_step)]
		...
		# print time_step, d_tot_new_cases_adult[(time_step)] + d_tot_new_cases_child[(time_step)]
		# go back thru while loop
		# next time step
		# travel again
		# S --> I --> R
	
	print sorted([(k, v) for k, v in d_metro_infected_child.items() if v > 0])
...
# run many (num_sims) instances of chain binomial simulation and return average epidemic size    
	...
	for sim in range(1,num_sims):
		...
		print 'sim, outbreak:', sim, outbreak_size
...
###################################################
#def write_csv_file(metro_ids, incidence_time_series_metro_child, incidence_time_series_metro_adult, tot_incidence_time_series_child, tot_incidence_time_series_adult):
#    
...
if __name__ == "__main__":
	...
	filename_metropop = os.path.normpath
	('%sAnne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/metedges.txt' %(start_dir))
	d_metropop, metro_ids = pop_func.import_metropop(filename_metropop, 2, 3)
	filename_air_network = os.path.normpath
	('%sAnne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/air_traffic_edgelist.txt' %(start_dir))
	air_network = read_edgelist_anne(filename_air_network)
	# READ US population data
	popfile = os.path.normpath('%sAnne_Bansal_lab/SDI_Data/totalpop_age.csv' %(start_dir))
	us_popdata = csv.reader(open(popfile, 'r'), delimiter = ',')
	...
	filename_germ_contact_data = os.path.normpath('%sAnne_Bansal_lab/Contact_Data/polymod_germany_contact_matrix_Mossong_2008.csv' %(start_dir))
	filename_germ_pop_data = os.path.normpath('%sAnne_Bansal_lab/UNdata_Export_2008_Germany_Population.csv' %(start_dir))
 ...
 #    # READ US population data
	...
	# dict_childpop, dict_adultpop = pop_func.pop_child_adult (dict_popdata, years)
	# # READ Germany contact data
	# filename_germ_contact_data = '/home/elee/Dropbox/Anne_Bansal_lab/Contact_Data/polymod_germany_contact_matrix_Mossong_2008.csv'
	# filename_germ_pop_data = '/home/elee/Dropbox/Anne_Bansal_lab/UNdata_Export_2008_Germany_Population.csv'

	# # Windows
	# filename_metropop = os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/metedges.txt')
	# d_metropop, metro_ids = pop_func.import_metropop(filename_metropop, 2, 3)
	# filename_air_network = os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/Python_Scripts/Modeling_Project/air_traffic_data/air_traffic_edgelist.txt')
	# air_network = read_edgelist_anne(filename_air_network)
	# # READ US population data
	# us_popdata = csv.reader(open(os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/SDI_Data/totalpop_age.csv'), 'r'),delimiter = ',')
	# dict_popdata, ages, years = pop_func.import_popdata(us_popdata, 0, 1, 2)
	# dict_childpop, dict_adultpop = pop_func.pop_child_adult (dict_popdata, years)
	# # READ Germany contact data
	# filename_germ_contact_data = os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/Contact_Data/polymod_germany_contact_matrix_Mossong_2008.csv')
	# filename_germ_pop_data = os.path.normpath('C:/Users/Elizabeth/Dropbox (Bansal Lab)/Anne_Bansal_lab/UNdata_Export_2008_Germany_Population.csv')
